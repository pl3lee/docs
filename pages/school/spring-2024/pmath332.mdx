...
<Proof>
    <ProofContent>
        Let $f(z) = u(x,y) + iv(x,y)$. Then $|f(z)| = \sqrt{u^2 + v^2} = M$, which means $u^2 + v^2 = M^2$. If we differentiate
        - with respect to $x$, then $2u u_x + 2v v_x = 0$
        - with respect to $y$, then $2u u_y + 2v v_y = 0$
        ...
        \end{align}
        $$
        If $f$ is analytic, (1) is equivalent to $uv_y - vu_y = 0$, and rearranging gives
        $$
        \begin{align*}
            -vu_y + uv_y &= 0 \tag{3}
        \end{align*}
        $$
        If we solve (3) for $u$, we get $u = \frac{vu_y}{v_y}$, and plugging this into (1) gives
        $$
        \begin{align*}
            \left(\frac{vu_y}{v_y}\right) v_x + vv_x &= 0 \\
        \end{align*}
        $$
    </ProofContent>
</Proof>