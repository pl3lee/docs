...
### Strings and Runes
Let's use the word `résumé` here since it has a bunch of weird characters (non ASCII). We can index a string using the same notation as an array. We use `%T` to print out a type of a variable in GO.
```go
...
```
We can see that the index skips 2. This is because the string is encoded in UTF-8, which uses a variable number of bytes to encode characters. The `é` character is encoded using two bytes, and has a unicode point number of 233. The underlying array that stores `myString` is an array of bytes
```go
//   r         é                   s         u         m         é
