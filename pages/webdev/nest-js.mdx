...
```
THen, in the generated `belt.guard.ts`, we can see that it implements the `CanActivate` interface, which has a `canActivate` method that returns a boolean. Then, we can 
